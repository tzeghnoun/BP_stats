...
hc_add_series_map(worldgeojson, oil_reserves[year == 2018], value = "oil_proved_reserves", joinBy = c('name','country'), name = "Oil Reserves (Thousand million barrels)")  %>% 
    hc_add_series(oil_reserves, hcaes(x = country, y = oil_proved_reserves, color = oil_proved_reserves), type = "pie", name = 'Oil Reserves (Thousand million barrels)')  %>% 
...
hc_add_series_map(worldgeojson, gas_reserves[year == 2018], value = "gas_proved_reserves", joinBy = c('name','country'), name = "Gas Reserves (Trillion cubic metres)")  %>% 
    hc_add_series(gas_reserves, hcaes(x = country, y = gas_proved_reserves, color = gas_proved_reserves), type = "pie", name = 'Gas Reserves (Trillion cubic metres)')  %>% 
    hc_colorAxis(stops = color_stops()) %>% 
...
```{r fig.height=3}
# Generate the dt
dt <- as.data.table(wide_to_long_1(59))
dt <- dt[, c("electricity_generation", "electricity_generation_") := .(round(as.numeric(electricity_generation_), 2), NULL)]
...
```{r fig.height=3}
# Get the dt
dt <- as.data.table(bp_stats$list.element[[60]])
names(dt) <- c("country", "oil_17", "ng_17", "coal_17", "nuclear_17", "hydroelectric_17", "renewables_17", "others_17", "total_17", "oil_18", "ng_18", "coal_18", "nuclear_18", "hydroelectric_18", "renewables_18", "others_18", "total_18")

# Get the index of the "Total World" row to remove the rows beyond it (rows containing comments)
index <- dt[, which(country %like% "Total World")]
dt <- na.omit(dt[1:index])
dt <- dt[, lapply(.SD, round, 2), .SDcols = 2:17, by = country]
